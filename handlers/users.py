...
def create_user():
    ...
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?)
        ''', (data['username'], hashed_password, data.get('email', ''), data.get('role', 'user')))

        conn.commit()
        user_id = cursor.lastrowid

        return jsonify({'message': 'Пользователь создан успешно', 'id': user_id})
...
def update_user(user_id):
    ...
    try:
        ...
        return jsonify({'message': 'Пользователь обновлен успешно'})
...
@users.route('/api/users/<int:user_id>', methods=['DELETE'])
def delete_user(user_id):
    if user_id == 1:  # Защита от удаления админа
    ...
    try:
        ...
        return jsonify({'message': 'Пользователь удален успешно'})
